...
$('.provisional_rep').change(function() {
		 // $("#search_advance").trigger('click');
		 $.each($("#invoice_type option:selected"), function () {
...
$("#search_advance").click(function() {
	...
	var business_unit_id   = $("#business_unit_id").val();
        var einvoice   = $("#einvoice").val();//($("#einvoice").is(':checked') === true) ? "1" : "";
	var provisional_rep = "";
	var selected = $("input[type='radio'][name='provisional_rep']:checked");
	if (selected.length > 0) {
	    provisional_rep = selected.val();
	}
	console.log(search_url);
	$.ajax({
		type: "POST",	
		url: site_base_url+search_url,
		// dataType: "json",
		data: "filter=filter"+"&einvoice="+einvoice+"&project="+project+"&customer="+customer+"&divisions="+divisions+"&practice="+practice+"&from_date="+from_date+"&to_date="+to_date+'&month_year_from_date='+month_year_from_date+"&month_year_to_date="+month_year_to_date+"&geography_ids="+geography_ids+"&project_location="+project_location+"&department="+department+"&business_unit_id="+business_unit_id+'&pm='+pm+'&am='+am+'&invoice_type='+invoice_type+'&invoice_status='+invoice_status+'&mail_status='+mail_status+'&customer_type='+customer_type+'&provisional_rep='+provisional_rep+"&"+csrf_token_name+'='+csrf_hash_token,
		beforeSend:function(){
			$('#results').empty();
...
function save_search() {
	...
	$.ajax({
		type: "POST",
		dataType: 'json',
		url: site_base_url+"invoice/save_search",
		cache: false,
		data: "search_name="+search_name+"&einvoice="+einvoice+"&is_default="+is_default+"&project="+project+"&customer="+customer+"&divisions="+divisions+"&project_location="+project_location+"&practice="+practice+"&from_date="+from_date+"&to_date="+to_date+"&month_year_from_date="+month_year_from_date+"&month_year_to_date="+month_year_to_date+"&pm="+pm+"&am="+am+'&'+csrf_token_name+'='+csrf_hash_token,
		beforeSend:function(){
		...
		success: function(response){
			...
			$.ajax({
				...
				url: site_base_url+search_url,
				cache: false,
				data: "&project="+project+"&customer="+customer+"&project_location="+project_location+"&divisions="+divisions+"&practice="+practice+"&from_date="+from_date+"&to_date="+to_date+"&month_year_from_date="+month_year_from_date+"&month_year_to_date="+month_year_to_date+"&pm="+pm+"&am="+am+'&'+csrf_token_name+'='+csrf_hash_token,
				success: function(data){
					$.unblockUI();
...
function show_search_results(search_id) {
	...
	$.ajax({
		type: "POST",
		url: site_base_url+search_url+"/"+search_id,
		cache: false,
		data: "&project="+project+"&customer="+customer+"&project_location="+project_location+"&divisions="+divisions+"&from_date="+from_date+"&to_date="+to_date+"&month_year_from_date="+month_year_from_date+"&month_year_to_date="+month_year_to_date+'&'+csrf_token_name+'='+csrf_hash_token,
		success: function(data){
			// $('#advance_search_results').html(data);
			$('#results').html(data);
			$('#search_advance').show();
	...
	switch(export_inv_type) {
		...
		case 'no_search':
			...
			var form = $('<form action="' + url + '" method="post">' +
			  ...
			  '<input id="am" type="hidden" name="am" value="'+am+'" />'+
			  '<input id="invoice_type" type="hidden" name="invoice_type" value="'+invoice_type+'" />'+
			  '<input id="invoice_status" type="hidden" name="invoice_status" value="'+invoice_status+'" />'+
			  '<input id="geography_ids" type="hidden" name="geography_ids" value="'+geography_ids+'" />'+
			  '<input id="project_location" type="hidden" name="project_location" value="'+project_location+'" />'+
			  '<input id="mail_status" type="hidden" name="mail_status" value="'+mail_status+'" />'+
			  '<input id="customer_type" type="hidden" name="customer_type" value="'+customer_type+'" />'+
			  '<input id="department" type="hidden" name="department" value="'+department+'" />'+
				'<input type="hidden" name="aging_report" value="'+aging_report+'" />' +
				'<input type="hidden" name="provisional_rep" value="'+provisional_rep+'" />' +
				'<input type="hidden" name="business_unit_id" value="'+business_unit_id+'" />' +
                                '<input type="hidden" name="einvoice" value="'+einvoice+'" />' +
