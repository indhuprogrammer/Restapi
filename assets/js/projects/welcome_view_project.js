...
*/
	...
	$(document).ready(function() {
		...
		$("#send_mail_tab").click(function() {
			if($("div").hasClass('mce-tinymce')){
				$(".mce-tinymce").remove();	
			}
			tinymce.init({
  		...
  		if(business_unit_id){
  			$.ajax({
  				...
  				success:function(data){
  					...
  					$('#practice').html('<option value="">Please Select</option>');
  	  ...
  	  business_unit_id = $('#business_unit_id').val();
  		if(department_id && business_unit_id){
  			$.ajax({
  				...
  				success:function(data){
  					$('#practice').html('<option value="">Please Select</option>'+data);
  					$('#practice').val($("#practice_hide").val());
  				}			
  			});
  		}
  	});
 ...
 ////////////////////////----------------------------X---------------------////////////////////////////
	...
	function loadPayment() 
	{
		...
		$.post( 
            ...
            "aaSorting": [[ 0, "desc" ]],
						...
						"bDestroy": true,
						"aoColumnDefs": [
							{ 'bSortable': false, 'aTargets': [ 0,1,5,6,7,8,9,10 ] },
	...
	function fullScreenCustomerLogs()
	{
		var fsl_height = parseInt($(window).height()) - 80;
		fsl_height = fsl_height + 'px';
		...
		params[csrf_token_name] = csrf_hash_token;
		$.post( 
			site_base_url+'project/getCustomerLogs/'+project_jobid,params,
			function(data) {
				if (data.error) {
					alert(data.errormsg);
				} else {
				...
				if(data.error != '') {
					...
					if(data.msg == 'File successfully uploaded!') {
						...
						$.each(data.res_file, function(i, item) {
							...
							// alert(res[0]+res[1]);	
							var name = '<div style="float: left; width: 100%;"><input type="hidden" name="file_id[]" value="'+res[0]+'"><span style="float: left;display:inline-flex;" id="'+res[0]+'">'+res[1]+'<a id="'+res[0]+'" class="del_file"> </a></span></div>';
...
function addURLtoJob() 
{
	...
	*/
	...
	function setPayment_term(){
		$('#resmsg_payment_term').empty();
		var payment_terms_list = $('#payment_terms_list').val();
		...
		$.ajax({
			type: 'POST',
			url: site_base_url+'project/set_payment_term/',
			dataType: 'json',
			data: 'payment_terms_list='+payment_terms_list+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
			success: function(data) {
				if (data.error == false) {
					$('#resmsg_payment_term').html("<span class='ajx_success_msg'>Payment term Updated</span>");
				} else {
					$('#resmsg_payment_term').show();
					$('#resmsg_payment_term').html("<span class='ajx_failure_msg'>"+data.error+"</span>");
				}
				$.unblockUI();
			}
		});
		$('#resmsg_payment_term').fadeOut(4000);
	 }
	//set the contract PO//
	 function setContrac(){
		$('#resmsg_setContractPO').empty();
		var contarct_po = $('#contarct_po').val();
		if(contarct_po == '') {
			$('#resmsg_err_cont').html("<span class='ajx_failure_msg'>Contract P.O is required</span>");
			return false;
		}
		...
		$.ajax({
			type: 'POST',
			url: site_base_url+'project/set_contarct_po/',
			dataType: 'json',
			data: 'contarct_po='+contarct_po+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
			success: function(data) {
				if (data.error == false) {
					$('#resmsg_setContractPO').html("<span class='ajx_success_msg'>Contract P.O Updated</span>");
					$('#resmsg_err_cont').html('')
				} else {
					$('#resmsg_setContractPO').show();
					$('#resmsg_setContractPO').html("<span class='ajx_failure_msg'>"+data.error+"</span>");
					$('#resmsg_err_cont').html('');
				}
				$.unblockUI();
			}
		});
		$('#resmsg_setContractPO').fadeOut(4000);
	 }
	//Set the Project Currency//
	function setProjectCurny()
	{
		$('#resmsg_currency').empty();
		var currency = $('#expect_worth_id').val();
		if(currency == '') {
			return false;
		}
	...
	function setProjectGeography(){
		$('#resmsg_geography').empty();
		var geography = $('#project_geography').val();
		if(geography == '') {
			return false;
		}
		...
		$.ajax({
			type: 'POST',
			url: site_base_url+'project/set_geography/',
			dataType: 'json',
			data: 'geography='+geography+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
			success: function(data) {
				if (data.error == false) {
					$('#resmsg_geography').html("<span class='ajx_success_msg'>Geography Updated</span>");
				} else {
					$('#resmsg_geography').show();
					$('#resmsg_geography').html("<span class='ajx_failure_msg'>"+data.error+"</span>");
				}
				$.unblockUI();
			}
		});
		$('#resmsg_geography').fadeOut(4000);
		
	}
  /*
	*@Method setBusinessUnit
	*@parameters business_unit_id, lead_id
	*@Use update department for particular leads
	*@Author eNoah - Mani.S
	*/
	function setBusinessUnit() {
		$('#resmsg_BusinessUnit').empty();
		...
		$.ajax({
			type: 'POST',
			url: site_base_url+'project/set_businessunit/',
			dataType: 'json',
			data: 'business_unit_id='+business_unit_id+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
			success: function(data) {
				if (data.error == false) {
					$('#resmsg_BusinessUnit').html("<span class='ajx_success_msg'>Status Updated</span>");
				} else {
					$('#resmsg_BusinessUnit').show();
					$('#resmsg_BusinessUnit').html("<span class='ajx_failure_msg'>"+data.error+"</span>");
				}
				$.unblockUI();
				setTimeout('timerfadeout()', 2000);
			}
	...
	*/
		...
		$( "#project-tabs" ).tabs({
			beforeActivate: function( event, ui ) {
				...
				switch(evnt_id){
					...
					break;
					case 'jv-tab-5-5':
						loadCustomerLogs(project_jobid);
					break;
                                        case 'jv-tab-8-7':
                                                loadQualityMetrics(project_jobid);
                                        break;
				}
			}
		});
	...
	function populateJobOverview()
	{
		...
		function paymentProfileEdit(eid,type) 
		{
			$(".payment-profile-view").show();
			$(".payment-invoice-view").hide();
			$('.modal-title').html('Edit Payment Terms');
			var jid = project_jobid;
			setTimeout('timerfadeout()', 2000);
			//var url = site_base_url+"project/payment_term_edit/"+eid+"/"+jid;
			var url = site_base_url+"project/invoice_term_edit/"+eid+"/"+jid+"/"+type;
    ...
    function paymentProfileEditCreditNote(eid,type) 
  	{
  		$(".payment-profile-view").show();
  		$(".payment-invoice-view").hide();
  		$('.modal-title').html('Edit Payment Terms');
  		...
  		setTimeout('timerfadeout()', 2000);
  		//var url = site_base_url+"project/payment_term_edit/"+eid+"/"+jid;
  		var url = site_base_url+"project/invoice_term_edit_credit/"+eid+"/"+jid+"/"+type;
  		$('#payment-profile-view').load(url);
  		setTimeout(function(){
  			 $('#mymodal').modal('show');
                         set_einvoice();
		...
		function paymentInvoiceProfileView(eid,type) 
		{
			$(".payment-profile-view").show();
			$(".payment-invoice-view").hide();
			$('.modal-title').html('View Payment Terms');
			var jid = project_jobid;
			setTimeout('timerfadeout()', 2000);
			var url = site_base_url+"project/invoice_term_view/"+eid+"/"+jid+"/"+type;
			$('#payment-profile-view').load(url);
			setTimeout(function(){
			   $('#mymodal').modal('show');
                           set_einvoice();
    ...
    function paymentInvoiceProfileViewCreditNote(eid,type) 
  	{
  		$(".payment-profile-view").show();
  		$(".payment-invoice-view").hide();
  		$('.modal-title').html('View Payment Terms');
  		var jid = project_jobid;
  		setTimeout('timerfadeout()', 2000);
  		var url = site_base_url+"project/invoice_term_view_credit/"+eid+"/"+jid+"/"+type;
  		$('#payment-profile-view').load(url);
  		setTimeout(function(){
  			 $('#mymodal').modal('show');
                         set_einvoice();
    ...
    function paymentProfileAddCreditNote(expId,type) {
  		$(".payment-profile-view").show();
  		$(".payment-invoice-view").hide();
  		$('.modal-title').html('Add Credit Note');
  		var jid = project_jobid;
  		setTimeout('timerfadeout()', 2000);
  		var url = site_base_url + "project/invoice_term_add_credit/" + jid + "/" + expId+ "/" + type;
  		$('#payment-profile-view').load(url);
  		setTimeout(function () {
  			$('#mymodal').modal({
  				backdrop: 'static',
		...
		function excelDownload(id){
			var url = site_base_url+"project/download_project/"+id;
      ...
      return false;
        ...
        function paymentInvoiceView(eid) 
		{   
		    $(".payment-profile-view").hide();			
			var jid = project_jobid;
		    setTimeout('timerfadeout()', 2000);
			var url = site_base_url+"project/invoice_term_edit/"+eid+"/"+jid;			
			$('#payment-invoice-view').load(url);
    		//alertMessageShows('<div style="color:red;">Please wait a moment....</div>','auto','35%','fixed','45%');
			setTimeout(function(){
			  //var html = $(".payment-invoice-view").html();				
				//$( "div.payment-invoice-view form" ).remove();				
			   //alertMessageShows('<br /><h2 class="job-title">Invoice</h2><p>'+html+'</p>','800px','10%','absolute','25%');
			   $('#payment-invoice-view').modal('show');
                           set_einvoice();
			},1000)			
		}
		
		function paymentProfileAdd(type) 
		{
			$.unblockUI();
			$(".payment-profile-view").show();
			$(".payment-invoice-view").hide();
			$('.modal-title').html('Add Payment Terms');
			var jid = project_jobid;
			setTimeout('timerfadeout()', 2000);		
			var url = site_base_url+"project/invoice_term_add/"+jid+"/"+type;
			$('#payment-profile-view').load(url);
			setTimeout(function(){
			   $('#mymodal').modal({
				backdrop:'static',   
				...
				show:true
                            ...
                            //E-invoice based business logic
                            var street_zip = $("#cutomer_address").find(':selected').data('street_zip');
                            var gstin = $("#cutomer_address").find(':selected').data('gstin');
                            //show hide street zip error
                            var entity = $("#entity").val();
                            if(entity == "1"&& street_zip === "show"){
                                $(".invoice_submit").hide();
	    ...
	    /*function alertMessageShows(val,width,topp,position,left){
			$.blockUI({
				message:''+val+'',
				css : { height: 'auto', 'cursor': 'auto', 'width': width, 'top': topp,'left':left, 'padding':'15px','position':position}
			});
		}
		function cancelDel() {
			$.unblockUI();
		...
		{   
		   $.blockUI({
			message:'<br /><h5>Are you sure you want to delete this file?</h5><div class="modal-confirmation overflow-hidden"><div class="buttons"><button type="submit" class="positive" onclick="formSubmitDelete('+eid+'); return false;">Yes</button></div><div class="buttons"><button type="submit" class="negative" style="margin-left: 6px;" onclick="cancelDel(); return false;">No</button></div></div>',
			css: {zIndex:'999999999'}
		  });
			
		}
		
		function formSubmitDelete(eid) 
		{	$.unblockUI();	
			var jid = project_jobid;
			setTimeout('timerfadeout()', 2000);
	...
	$.ajax({
		...
		success: function(data) {
			...
			$.blockUI({
				...
				//css: { border: '2px solid #999',color:'#333',padding:'8px',top: ($(window).height()) /2 + 'px',left: ($(window).width()) /2 + 'px',width: '500px',position: 'absolute', maxHeight: '450px', 'overflow-y':'auto', 'overflow-x':'hidden'}	
                centerY: true,
				centerX: true,
				css:{
					position: 'fixed',
					margin: 'auto',
					top: '28%',
					border: '2px solid #999',color:'#333',padding:'8px',
				}				
			});
...
function select_files() {
	...
	jQuery.each( $(".attach_file:checked"), function( i, val ) {
		...
		id += filename[0] + ',';
		data += '<div style="float: left; width: 100%;"><input type="hidden" name="file_id[]" value="'+filename[0]+'"><span style="float: left;display:inline-flex;" id="'+filename[0]+'">'+filename[1]+' <a id="'+filename[0]+'" class="del_file"> </a></span></div>';
	});
...
function editFolderPermissions(lead_id)
{
	...
	$.get(site_base_url+'project/get_folder_permissions_ui_for_a_project', {'lead_id':lead_id, 'parent_folder_id': parent_folder_id}, function(data)
	{
...
$('#mymodal_invoice').on('hidden.bs.modal', function (e) {
	//location.reload(true);
});

function InvoiceEmail(id,downloadfile,expect_id,lead_name,milestone,date) {
	console.log(date);
	var trimStr = downloadfile;
	//var download = $("#download").val();
	console.log(milestone);
	console.log(id);
	console.log(downloadfile);
	var pdf = trimStr.replace(' ','%20');
	var vdate = date.replace(' ','');
	console.log(pdf);
	console.log(expect_id);
	console.log(lead_name);
	//var milestone = 'milestone';
	$(".milestone_invoice").show();
	$('.modal-title1').html('Client Invoice email Notification');
	//var url = site_base_url+"project/send_email_customer/"+id+"/"+expect_id+"/"+lead_name+"/"+downloadfile;
	var url = site_base_url+"project/send_email_customer/"+id+"/"+expect_id+"/"+pdf+"/"+lead_name+"/"+vdate;
	$('#milestone_invoice').load(url);	
		   setTimeout(function(){
...
//    var params  = {};
//    params[csrf_token_name] = csrf_hash_token;
//    params['path'] = downloadfile;
//    params['expect'] = expect_id;
//    params['lead'] = lead_name;
//    params['mile_stone'] = milestone;
//    $.ajax({
// 	   url : site_base_url + 'project/send_email_customer/'+id,
// 	   cache: false,
// 	   type: "POST",
// 	   data:params,
// 	   success : function(response) {
// 		console.log(response);
// 		   $('#milestone_invoice').load(response);	
// 		   setTimeout(function(){
// 					  $('#mymodal_invoice').modal('show');
// 		   },500)
// 	   }
...
function setDevelopers() {
	...
	var developers = $('#developers').val();
        var url = site_base_url+'project/set_developers/';
	if(!developers) {
		$('#resmsg7').show();
		$('#resmsg7').html("<span class='ajx_failure_msg'>Please select Developers!.</span>");
		return false;
	}
...
//	show_processing();
        ...
        // Set Developers
        setDevelopersRequest(url, developers);
...
function setReporter() {
	...
	if(!reporters) {
		$('#resmsg4').show();
		$('#resmsg4').html("<span class='ajx_failure_msg'>Please select Reporters!.</span>");
		return false;
	}
	...
	$.ajax({
		type: 'POST',
		url: site_base_url+'project/set_reporters/',
		dataType: 'json',
		data: 'reporters='+reporters+'&project_code='+project_code+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
		success: function(data) {
			if (data.error == false) {
				$('#resmsg4').show();
				$('#resmsg4').html("<span class='ajx_success_msg'>Reporters Updated.</span>");
				show_updating();
			} else {
				$('#resmsg4').show();
				$('#resmsg4').html("<span class='ajx_failure_msg'>"+data.error+".</span>");
				$.unblockUI();
			}
		}
	});
...
// Set Reporters to the Project
function setReporter(role_id) {	
    $('#reporter_error').hide();
    var reporter = $('#reporter').val();
    var existReporters = $('#exist_reporters').val();
    if(!reporter && !existReporters) {
            $('#reporter_error').show();
            $('#reporter_error').html("<span class='ajx_failure_msg'>Please select Reporter!.</span>");
            return false;
    }
    reporter = reporter ? reporter : "";
    setUserRequest(role_id,reporter,existReporters,'reporter_error');
}

// Set Auditors to the Project
function setAuditor(role_id) {	
    $('#auditor_error').hide();
    var auditor = $('#auditor').val();
    var existAuditors = $('#exist_auditors').val();
    if(!auditor && !existAuditors) {
            $('#auditor_error').show();
            $('#auditor_error').html("<span class='ajx_failure_msg'>Please select Auditor!.</span>");
            return false;
    }
    auditor = auditor ? auditor : "";
    setUserRequest(role_id,auditor,existAuditors,'auditor_error');
}

// Set Function Head to the Project
function setFh(role_id) {	
    $('#fh_error').hide();
    var fh = $('#function_head').val();
    var existFh = $('#exist_fh').val();
    if(!fh && !existFh) {
            $('#fh_error').show();
            $('#fh_error').html("<span class='ajx_failure_msg'>Please select Function Head!.</span>");
            return false;
    }
    fh = fh ? fh : "";
    setUserRequest(role_id,fh,existFh,'fh_error');
}

// Set CISO to the Project
function setCiso(role_id) {	
    $('#ciso_error').hide();
    var ciso = $('#ciso').val();
    var existCiso = $('#exist_ciso').val();
    if(!ciso && !existCiso) {
            $('#ciso_error').show();
            $('#ciso_error').html("<span class='ajx_failure_msg'>Please select CISO!.</span>");
            return false;
    }
    ciso = ciso ? ciso : "";
    setUserRequest(role_id,ciso,existCiso,'ciso_error');
}

// Request to Set the users to project
function setUserRequest(role_id,user_ids,exist_user_ids,error_tag) {
    
    show_processing();

    $.ajax({
            type: 'POST',
            url: site_base_url+'project/setUsersToProject/',
            dataType: 'json',
            data: 'role_id='+role_id+'&user_ids='+user_ids+'&exist_user_ids='+exist_user_ids+'&project_code='+project_code+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
            success: function(data) {
                    if (data.error == false) {
                            $('#'+error_tag).show();
                            $('#'+error_tag).html("<span class='ajx_success_msg'>User Updated.</span>");
                            show_updating();
                    } else {
                            $('#'+error_tag).show();
                            $('#'+error_tag).html("<span class='ajx_failure_msg'>"+data.error+".</span>");
                            $.unblockUI();
                    }
            }
    });
...
function setAuditee() {
	...
	if(!auditee) {
		$('#resmsg6').show();
		$('#resmsg6').html("<span class='ajx_failure_msg'>Please select Auditee!.</span>");
		return false;
	}
	...
	$.ajax({
		type: 'POST',
		url: site_base_url+'project/set_auditee/',
		dataType: 'json',
		data: 'auditee='+auditee+'&project_code='+project_code+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
		success: function(data) {
			if (data.error == false) {
				$('#resmsg6').show();
				$('#resmsg6').html("<span class='ajx_success_msg'>Auditee Updated.</span>");
				show_updating();
			} else {
				$('#resmsg6').show();
				$('#resmsg6').html("<span class='ajx_failure_msg'>"+data.error+".</span>");
				$.unblockUI();
			}
		}
	});
...
// Load popup for adding Group
function select_from_group(){
    $('#group_title').html('Select Group');
    setTimeout('timerfadeout()', 2000);		
    var url = site_base_url+"project/load_group_popup/";
    $('#set-group-view').load(url);
    setTimeout(function(){
        $.blockUI({
            message: $('#select_from_group_modal'),
            css: { border: '2px solid #999',color:'#333',padding:'8px',top: ($(window).height()) /3 + 'px',left: ($(window).width() - 400) /2 + 'px',width: '500px',position: 'absolute', maxHeight: '450px', 'overflow-y':'auto', 'overflow-x':'hidden'}			
        });
    },500);	
}
...
function get_group_members(group_id) {
    setTimeout('timerfadeout()', 2000);		
    var url = site_base_url+"project/get_group_members/"+group_id;
    $('#group_members_table').load(url);
}

// Set Group members as developers
function set_group_members() {
    var group_member_ids = $("#group_member_ids").val();
    var url = site_base_url+'project/set_developers_from_group/';  
    // Set Developers
    setDevelopersRequest(url, group_member_ids);
    setTimeout('timerfadeout()', 3000);
    return false;
}

// Set developers request
function setDevelopersRequest(url, developers) {
    $.ajax({
        type: 'POST',
        url: url,
        dataType: 'json',
        data: 'developers='+developers+'&project_code='+project_code+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
        success: function(data) {
            if (data.error == false) {
                    $('#resmsg7').show();
                    $('#resmsg7').html("<span class='ajx_success_msg'>Developers Updated.</span>");
                    show_updating();
            } else {
                    $('#resmsg7').show();
                    $('#resmsg7').html("<span class='ajx_failure_msg'>"+data.error+".</span>");
                    $.unblockUI();
            }
        }
    });
...
function getTrackerList(obj){
    var moduleId = obj.value;
    var projectId = $("#compliance_form #project_id").val();
    var moduleName = $("#compliance_form #module_id option:selected").text();
    $("#compliance_form #module_name").val(moduleName);
    if (moduleId && projectId) {
        var url = site_base_url+'compliance/getModuleTrackers'; 
        $.ajax({
            type: 'get',
            url: url,  
            data:{'moduleId':moduleId,'projectId':projectId,'ci_csrf_token':$('token').val()},	  
            success: function(resultData) { 
                $('#compliance_form #tracker_id').html('<option value="">Select Tracker</option>'+resultData); 
                $("#compliance_form #tracker_id").trigger("liszt:updated");  
            }
        });
    } else {
        $('#compliance_form #tracker_id').html('<option value="">Select Tracker</option>');
        $("#compliance_form #tracker_id").trigger("liszt:updated");  
    }
    return false;
}

// Tracker change process
function trackerChange(obj){
    var trackerName = $("#compliance_form #tracker_id option:selected").text();
    var defaultStatus = $('#compliance_form #tracker_id option:selected').attr('status');
    $("#compliance_form #tracker_name").val(trackerName);
    $("#compliance_form #default_status").val(defaultStatus);
    return false;
}

// Trigger Create ticket
function triggerCreateTicketBtn() {
...
function setProjectLocation(){
	...
	$.ajax({
		type: 'POST',
		url: site_base_url+'welcome/set_projectLocation/',
		dataType: 'json',
		data: 'project_location='+project_location+'&lead_id='+curr_job_id+'&'+csrf_token_name+'='+csrf_hash_token,
		success: function(data) {
			if (data.error == false) {
				$('#resmsg_location').html("<span class='ajx_success_msg'>Project Location Updated</span>");
			} else {
				$('#resmsg_location').show();
				$('#resmsg_location').html("<span class='ajx_failure_msg'>"+data.error+"</span>");
			}
			$.unblockUI();
		}
	});
	$('#resmsg_location').fadeOut(4000);
	
}

function loadQualityMetrics(id) {
	var params = {};
	params['estimate_hour'] = $('#project-estimate-hour').val();
	params['actual_hour'] = $('#actualEff').val();
	params['actual_end_date'] = $('#actual-project-due-date').val();
	params['planned_end_date'] = $('#project-due-date').val();
	params['planned_start_date'] = $('#project-start-date').val();

	params[csrf_token_name] = csrf_hash_token;
	/*$.post( 
		site_base_url+'quality_metrics/index/'+id,params,
		function(data) {
		$('#quality_metrics_new').html(data);
		}
	); */       
        $.ajax({
		type: 'POST',
		url: site_base_url+'quality_metrics/index/'+id,
		//dataType: 'json',
		data: params,
		beforeSend: function() {
                    $('#quality_metrics_new').html('<div style="margin:20px;" align="center">Loading Content.<br><img alt="wait" src="'+site_base_url+'assets/images/ajax_loader.gif"><br>Thank you for your patience!</div>');
		},
		success:function(data) {
                    console.log('In');
                    $('#quality_metrics_new').html(data);
