...
$('#document_tbl').delegate( '#addRow', 'click', function () {
	...
	var obj = $(thisRow).clone().insertAfter(thisRow);
		...
		obj.find(".location_type").val("");
		obj.find("#other_location_hidden").val("");
		obj.find("#position_hidden").val("");
		obj.find(".phone").val("");
		obj.find(".email").val("");
		obj.find(".skype").val("");
...
$('#document_tbl2').delegate( '.del_file', 'click', function () {
	...
	if( $(this).attr('hyperid') !=0 ) {
		...
		if(x==true)
		{
			...
			p_count = $('.position').length;
			if(p_count > 1){
				deleteOtherLocation($(this));
				$(thisRow).remove();
			}else{
				alert('You cannot remove the row');
			}
		}
	} else {
		p_count = $('.position').length;
...
function deleteOtherLocation(ths) {
		var baseurl = $('.hiddenUrl').val();
		var asset_id = $('#asset_id').val();
		var other_location = ths.parent().parent().find('#other_location_hidden').val();
		var position = ths.parent().parent().find('#position_hidden').val();

		var params  = {other_location: other_location,position:position,asset_id:asset_id}
		params[csrf_token_name] = csrf_hash_token;
		$.ajax({
		url : baseurl + 'asset_register/ajax_delete_location',
		data: params,
		type: "POST",
		dataType: 'json',
		async: false,
