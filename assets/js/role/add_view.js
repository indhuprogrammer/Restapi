...
function chk_role_name() {
	...
	var viewIsChecked = $('.view:checked').length;
	if (role_name == "" || viewIsChecked == 0) {
		if (role_name == ""){
			$('#role_msg').show();
		...
		if (viewIsChecked == 0){
			$('#module_msg').show();
			$('#module_msg').html("<span class='ajx_failure_msg'>Please select any Modules for the Role</span>");
		}
		return false;
	} else {
		$.ajax({
