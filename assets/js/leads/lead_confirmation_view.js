...
function ajxLoc(ths) {
	$(ths).nextAll("#addLocation").slideToggle("slow");
}
...
function ajxSaveLoc(ths, stId) {
	$(document).ready(function () {
		...
		*/
		...
		if ($(ths).prev("#newlocation").val() == "") {
			alert("Location Required.");
		}
		else {
			// var stId = $("#add1_state").val();
			var newLoc = $(ths).prev("#newlocation").val();
			getLoc(newLoc, stId, ths);
		}
		function getLoc(newLoc, stId, ths) {
			params = { stateid: stId, location_name: newLoc };
			params[csrf_token_name] = csrf_hash_token;

			$.ajax({
				url: 'customers/getLocRes/' + newLoc + '/' + stId,
				cache: false,
				success: function (response) {
					if (response == 'userOk') {
						$.post("regionsettings/location_add_ajax", params,
							function (info) {
								$(ths).parent().parent().children('select').append(info);
								$(ths).prev("#newlocation").val('');
								$(ths).parent().parent().children("#addLocation").hide();
							});
						$("#addstate").hide();
...
$('#copy').on("click", function () {
	...
	//	$("#ui-accordion-accordion-panel-2").hide();
		...
		$("[name='ship_block']").val($("[name='ship_block_copy']").val());
		$("[name='ship_street_po']").val($("[name='copy_ship_street_po']").val());
		$("[name='ship_street_no']").val($("[name='copy_ship_street_no']").val());
		$("[name='ship_city']").val($("[name='copy_ship_city']").val());
		$("[name='ship_suburb']").val($("[name='copy_ship_suburb']").val());
		$("[name='ship_zipcode']").val($("[name='copy_ship_zipcode']").val());
	}
...
function getCountry_ship(val,id) {
	...
	$('#country_row_ship').load(sturl, function(data){
		//var bill_country = 	$("add1_country").val();
		var shipcountry = $("#ship_country").val();
		var billcountry  = $("#add1_country").val();
...
function update_customer_details(){
    ...
    // if ($('#tele1').val() == '') {
    //     err1.push('Tele1 is required');
	// 	$('#tele1_err').html('Tele1 is required');
	// }
	
	// if ($('#phone').val() == '') {
    //     err1.push('Phone number is required');
	// 	$('#phone_err').html('Phone number is required');
	// }
	
	// if ($('#emailval').val() == '') {
    //     err1.push('Email is required');
	// 	$('#email_2_err').html('Email is required');
	// }
    // if ($('#www').val() == '') {
    //     err1.push('Website is required');
	// 	$('#www_err').html('Website is required');
	// }
