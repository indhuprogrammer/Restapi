...
$(document).ready(function() {
	...
	$(document).delegate('button#reg_cancel','click',function() { 
		window.location.href= site_base_url+"georegionsettings";
		return false;
	});
});
...
$(document).ready(function() {
	...
	$(document).delegate('#btnAddCountry','click',function() {
		...
		var country_code  = $('#country_country_code').val();
			if(country_code == ""){
				$('td#error3.error').show();
				return false;
		}
		$('#country_form').submit();
    });
...
function checkStatus(id) {
	...
	$.ajax({
		type: "POST",
		url: site_base_url+'georegionsettings/ajax_check_status_rcsl/',
		dataType:"json",                                                                
		data: formdata,
		cache: false,
		beforeSend:function(){
		...
		success: function(response) {
				...
				$.blockUI({
					message:'<br /><h5>Are You Sure Want to Delete this Geographical Region?<br /></h5><div class="modal-confirmation overflow-hidden"><div class="buttons"><button type="submit" class="positive" onclick="processDeleteRegion('+id+'); return false;">Yes</button></div><div class="buttons"><button type="submit" class="negative" onclick="cancelDel(); return false;">No</button></div></div>',
					css:{width:'440px'}
				});
...
function processDeleteRegion(id) {
	window.location.href = site_base_url+'georegionsettings/region_delete/delete/'+id;
}
