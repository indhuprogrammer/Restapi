...
if ( ! function_exists('get_folder_access_subcription'))
{
	function get_folder_access_subcription($lead_id, $folder_id, $user_id)
	{
		$CI  = get_instance();
		$cfg = $CI->config->item('crm'); // load config		
		$CI->db->select('access_type');
		$CI->db->where(array('sub_id' => $lead_id,'folder_id' => $folder_id,'role_id' => $user_id));
		$sql = $CI->db->get($cfg['dbpref'].'subscription_folder_access');
		$res = $sql->row_array();
		return $res['access_type'];
	}
}
if ( ! function_exists('get_user_access_subcription'))
{
	function get_user_access_subcription($lead_id, $user_id, $type)
	{
		$CI  = get_instance();
		$cfg = $CI->config->item('crm'); // load config	
        $val= '';		
		$CI->db->select('created_by,created_user_id');
		$CI->db->where(array('hostingid' => $lead_id));
		$sql = $CI->db->get($cfg['dbpref'].'hosting');
	    $res = $sql->row_array();
		if($res['created_by'] == $user_id){
			if($type == 'read'){
				$val =1;
			}else if($type == 'write'){
				$val =2;
		...
		if($res['created_user_id'] == $user_id){
			if($type == 'read'){
				$val =1;
			}else if($type == 'write'){
				$val =2;
...
if ( ! function_exists('sub_check_is_root'))
{
	function sub_check_is_root($lead_id, $folder_id)
	{
		$CI  = get_instance();
		$cfg = $CI->config->item('crm'); // load config		
		$CI->db->select('*');
		$CI->db->where(array('sub_id' => $lead_id,'folder_id' => $folder_id));
		$sql = $CI->db->get($cfg['dbpref'].'file_management_subscription');
		$res = $sql->row_array();
		// echo $CI->db->last_query(); exit;
		if($res['parent']==0){
			return 'root';
...
function package_name_new($package){  
	...
	foreach($package as $packageid){ 
	 ...
	 $CI->db->where('package_id',$packageid);
	 $CI->db->from($cfg['dbpref']. 'package');
	 $query = $CI->db->get();
	 $res = $query->row();
...
function package_name_old($package){	
    $CI = get_instance();
	$cfg = $CI->config->item('crm'); // load config
	$package_name=array();
	foreach($package as $p){ 
	 $CI->db->select('package_name');
	 $CI->db->where('package_id',$p['packageid_fk']);
	 $CI->db->from($cfg['dbpref']. 'package');
	 $query = $CI->db->get();
	 $res = $query->row();
	 $package_name[] = $res->package_name;    
...
function getCustomerValue($id){
	...
	$cfg = $CI->config->item('crm'); // load config
	$CI->db->select('c.*,cc.*');
	$CI->db->from($cfg['dbpref'] . 'customers as c');
	$CI->db->join($cfg['dbpref'] . 'customers_company as cc', 'cc.companyid = c.company_id');
	$CI->db->where_in('c.custid', $id);
	$customer = $CI->db->get();
		...
		if ($customer->num_rows() > 0) {
			...
			return "{$cust['customer_name']} {$company}";
		} else {
			return FALSE;
		}
 ...
 function getSubscriptionType($id){
	    $CI = get_instance();
    	$cfg = $CI->config->item('crm'); // load config
        $CI->db->select('subscriptions_type_name');
        $CI->db->from($cfg['dbpref'] . 'subscriptions_type');
        $CI->db->where('subscriptions_type_id', $id);
        $accounts = $CI->db->get();
        $list = $accounts->row();
        return $list->subscriptions_type_name;
 }
 
 
 function getDomainStatusValue($id){
	 $CI = get_instance();
     $cfg = $CI->config->item('crm');
	 return $cfg['domain_status'][$id];
	
 }
 function getSubOwnerValue($id){
	 $CI = get_instance();
 ...
 function getSSlValue($id){
	 ...
	 if($id==2){
		return 'Dedicated SSL'; 
	 }	 
 }
  function getNotificationValue($id){
	 if($id==0){
		return 'Yes';
	 }
	 if($id==1){
 ...
 function getTrackStatusValue($id){
	 $CI = get_instance();
	 $cfg = $CI->config->item('crm'); // load confi
	 return $cfg['payment_tracking'][$id];
 }
 function geSubUserValue($ids){
	 $CI = get_instance();
	 $cfg = $CI->config->item('crm'); // load config
	 $res=array();
	 $ids =explode(',',$ids);
