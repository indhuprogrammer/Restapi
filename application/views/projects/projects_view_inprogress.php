...
if(!empty($db_fields) && count($db_fields)>0){
	$td_chk = true;
	$td_cn = $td_cp = $td_pt = $td_pg = $td_ct = $td_plo = $td_rag = $td_ph = $td_bh = $td_ih = $td_nbh = $td_tuh = $td_ev = $td_pv = $td_uc = $td_dc = $td_oc = $td_ir = $td_contrib = $td_pl = $td_plp = 'style="display: none;"';
}
?>
<?php
	$milestone_content = '';
	...
	if (is_array($pjts_data) && count($pjts_data) > 0) {
		...
		foreach($pjts_data as $record){	
			...
			$project_geography  = !empty($record['project_geography']) ? $record['project_geography'] : '-'; 
			$customer_type  = !empty($record['customer_type']) ? $record['customer_type'] : '-'; 
			$project_location  = !empty($record['project_location']) ? $record['project_location'] : '-'; 
			...
			//To show project approval screen update
			$access_1 = access_action(array('approver_1'));
			...
			if($bill_type == 1) {
				...
				//To  edit the project approval screen
				if(!empty($approver_1) && !empty($approver_2) && (in_array($this->userdata['userid'],$approver_1)|| in_array($this->userdata['userid'],$approver_2)) ){
					$milestone_content .= "<a title='Project Approval Screen' class='view-icon' href='project_approval/view_project_approval/".$record['lead_id']."?pjtlist=1'><img src='assets/img/add-details.png' alt='Project Approval Screen'></a> ";
				}
				...
				if($td_chk == false) {
					...
					$milestone_content .= "<td>".$project_geography."</td>";
					$milestone_content .= "<td>".$customer_type."</td>";                    			$milestone_content .= "<td>".$project_location."</td>";		
					...
					if(($td_chk == true) && in_array('PG', $db_fields)) { $td_pg = 'style="display: table-cell;"'; }
					$milestone_content .= "<td ".$td_pg.">".$project_geography."</td>";
					if(($td_chk == true) && in_array('CT', $db_fields)) { $td_ct = 'style="display: table-cell;"'; }
					$milestone_content .= "<td ".$td_ct.">".$customer_type."</td>";
					if(($td_chk == true) && in_array('RAG', $db_fields)) { $td_plo = 'style="display: table-cell;"'; }		
					$milestone_content .= "<td ".$td_plo.">".$project_location."</td>";
				...
				//To  edit the project approval screen
				if(!empty($approver_1) && !empty($approver_2) && (in_array($this->userdata['userid'],$approver_1)|| in_array($this->userdata['userid'],$approver_2)) ){
					$monthly_content .= "<a title='Project Approval Screen' class='view-icon' href='project_approval/view_project_approval/".$record['lead_id']."?pjtlist=1'><img src='assets/img/add-details.png' alt='Project Approval Screen'></a> ";
				}
				if($this->session->userdata('delete')==1) {
				...
				if($td_chk == false) {
					...
					$monthly_content .= "<td>".$project_geography."</td>";	
					$monthly_content .= "<td>".$customer_type."</td>";  
					$monthly_content .= "<td>".$project_location."</td>";  
					...
					if(($td_chk == true) && in_array('PG', $db_fields)) { $td_pg = 'style="display: table-cell;"'; }
					$monthly_content .= "<td ".$td_pg.">".$project_geography."</td>";
					if(($td_chk == true) && in_array('CT', $db_fields)) { $td_ct = 'style="display: table-cell;"'; }
                    $monthly_content .= "<td ".$td_ct.">".$customer_type."</td>";
					if(($td_chk == true) && in_array('RAG', $db_fields)) { $td_plo = 'style="display: table-cell;"'; }
					$monthly_content .= "<td ".$td_plo.">".$project_location."</td>";
					if(($td_chk == true) && in_array('RAG', $db_fields)) { $td_rag = 'style="display: table-cell;"'; }
...
<div class="page-title-head">
	...
	<div class="section-right">
		<div class="buttons export-to-excel">
			<button type="button" onclick="confirmaMoveLeadsToProject(); return false;">
			Add new Project
			</button>
	...
	<thead>
		<tr>
			...
			<?php if($td_chk == false) { ?>
                ...
                <th title="Project Geography">PG</th>		
				<th title="Customer Type">CT</th>    
				<th title="Project Location">PLO</th>                			
			...
			<?php } else { ?>
                ...
                <th <?php echo $td_pg; ?> title="Project Geography">PG</th>		
				<th <?php echo $td_ct; ?> title="Customer Type">CT</th> 
				<th <?php echo $td_plo; ?> title="Project Location">PLO</th>     
		...
		<thead>
			<tr>
				...
				<?php } else { ?>
                    ...
                    <th <?php echo $td_pg; ?> title="Project Geography">PG</th>		
					<th <?php echo $td_ct; ?> title="Customer Type">CT</th>  
					<th <?php echo $td_ct; ?> title="Project Location">PLO</th>  
					<th <?php echo $td_rag; ?> title="RAG Status">RAG</th>
					<th <?php echo $td_ph; ?> title="Planned Hour">PH</th>
					<th <?php echo $td_bh; ?> title="Billable Hour">BH</th>
					<th <?php echo $td_ih; ?> title="Internal Hour">IH</th>
...
<fieldset>
	...
	<div align="left" style="background: none repeat scroll 0 0 #3b5998;">
		...
		<div class="legend">
			...
			<div class="pull-left"><strong>PG </strong> - Project Geography</div>
			<div class="pull-left"><strong>CT </strong> - Customer Type</div>
			<div class="pull-left"><strong>PLO </strong> - Project Location</div>
...
function confirmaMoveLeadsToProject()
{
	var params = {};
	$.post( 
		site_base_url+'project/addProject/', {'<?php echo $this->security->get_csrf_token_name(); ?>':'<?php echo $this->security->get_csrf_hash(); ?>'},
		function(data) {
			if (data.error) {
			} else {
				$('.project_new').html(data);
			}
		}
	);	
	...
	// $('html, body').animate({ scrollTop: $(".project_new").offset().top }, 1000);
	// $( ".project_new" ).parent().addClass( "assign-user" );
}
</script>
