...
if(!empty($resdata)) {
	foreach($resdata as $rec) {
		if(isset($tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['hour'])) {
			$tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['hour'] += $rec->duration_hours;
		} else {
			$tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['hour'] = $rec->duration_hours;
		}
		if(isset($tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['cost'])){
			$tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['cost'] += $rec->resource_duration_cost;
			$tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['directcost'] += $rec->resource_duration_direct_cost;
		} else {
			$tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['cost'] = $rec->resource_duration_cost;
			$tbl_data[$rec->georegion_name][$rec->skill_name][$rec->empname][$rec->project_code]['directcost'] = $rec->resource_duration_direct_cost;
		}
		...
		//for sub total
		if(isset($sub_tot[$rec->georegion_name]['sub_tot_hour'])){
			$sub_tot[$rec->georegion_name]['sub_tot_hour'] +=  $rec->duration_hours;
		} else {
			$sub_tot[$rec->georegion_name]['sub_tot_hour'] =  $rec->duration_hours;
		}
		if(isset($sub_tot[$rec->georegion_name]['sub_tot_cost'])){
			$sub_tot[$rec->georegion_name]['sub_tot_cost'] +=  $rec->resource_duration_cost;
		} else {
			$sub_tot[$rec->georegion_name]['sub_tot_cost'] =  $rec->resource_duration_cost;
		}
		if(isset($sub_tot[$rec->georegion_name]['sub_tot_directcost'])){
			$sub_tot[$rec->georegion_name]['sub_tot_directcost'] +=  $rec->resource_duration_direct_cost;
		} else {
			$sub_tot[$rec->georegion_name]['sub_tot_directcost'] =  $rec->resource_duration_direct_cost;
		}
		if(isset($skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_hour']))
		$skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_hour'] += $rec->duration_hours;
		else 
		$skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_hour'] = $rec->duration_hours;
		
		if(isset($skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_cost']))
		$skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_cost'] += $rec->resource_duration_cost;
		else 
		$skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_cost'] = $rec->resource_duration_cost;
	
		if(isset($skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_directcost']))
		$skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_directcost'] += $rec->resource_duration_direct_cost;
		else 
		$skil_sub_tot[$rec->georegion_name][$rec->skill_name]['skil_sub_tot_directcost'] = $rec->resource_duration_direct_cost;
		//for sub total
		
		//for practicewise - sorting-hour
		if(isset($sub_tot_hr[$rec->georegion_name]))
		$sub_tot_hr[$rec->georegion_name] +=  $rec->duration_hours;
		else
		$sub_tot_hr[$rec->georegion_name] =  $rec->duration_hours;
		//for practicewise sorting-cost
		if(isset($sub_tot_cst[$rec->georegion_name]))
		$sub_tot_cst[$rec->georegion_name] +=  $rec->resource_duration_cost;
		else
		$sub_tot_cst[$rec->georegion_name] =  $rec->resource_duration_cost;
		//for practicewise sorting-directcost
		if(isset($sub_tot_directcst[$rec->georegion_name]))
		$sub_tot_directcst[$rec->georegion_name] +=  $rec->resource_duration_direct_cost;
		else
		$sub_tot_directcst[$rec->georegion_name] =  $rec->resource_duration_direct_cost;
		//for skillwise - sorting-hour
		if(isset($skil_sort_hr[$rec->georegion_name][$rec->skill_name]))
		$skil_sort_hr[$rec->georegion_name][$rec->skill_name] += $rec->duration_hours;
		else 
		$skil_sort_hr[$rec->georegion_name][$rec->skill_name] = $rec->duration_hours;
		//for skillwise - sorting-cost
		if(isset($skil_sort_cst[$rec->georegion_name][$rec->skill_name]))
		$skil_sort_cst[$rec->georegion_name][$rec->skill_name] += $rec->resource_duration_cost;
		else 
		$skil_sort_cst[$rec->georegion_name][$rec->skill_name] = $rec->resource_duration_cost;
		//for skillwise - sorting-cost
		if(isset($skil_sort_directcst[$rec->georegion_name][$rec->skill_name]))
		$skil_sort_directcst[$rec->georegion_name][$rec->skill_name] += $rec->resource_duration_direct_cost;
		else 
		$skil_sort_directcst[$rec->georegion_name][$rec->skill_name] = $rec->resource_duration_direct_cost;
		//for userwise - sorting-hour
		if(isset($user_hr[$rec->georegion_name][$rec->skill_name][$rec->empname]))
		$user_hr[$rec->georegion_name][$rec->skill_name][$rec->empname] += $rec->duration_hours;
		else 
		$user_hr[$rec->georegion_name][$rec->skill_name][$rec->empname] = $rec->duration_hours;
		//for userwise - sorting-cost
		if(isset($user_cst[$rec->georegion_name][$rec->skill_name][$rec->empname]))
		$user_cst[$rec->georegion_name][$rec->skill_name][$rec->empname] += $rec->resource_duration_cost;
		else 
		$user_cst[$rec->georegion_name][$rec->skill_name][$rec->empname] = $rec->resource_duration_cost;
		//for userwise - sorting-cost
		if(isset($user_directcst[$rec->georegion_name][$rec->skill_name][$rec->empname]))
		$user_directcst[$rec->georegion_name][$rec->skill_name][$rec->empname] += $rec->resource_duration_direct_cost;
		else 
		$user_directcst[$rec->georegion_name][$rec->skill_name][$rec->empname] = $rec->resource_duration_direct_cost;
		...
		//usercount
		if (!in_array($rec->empname, $pr_usercnt[$rec->georegion_name]))
		$pr_usercnt[$rec->georegion_name][] = $rec->empname;
	
		if (!in_array($rec->empname, $sk_usercnt[$rec->georegion_name][$rec->skill_name]))
		$sk_usercnt[$rec->georegion_name][$rec->skill_name][] = $rec->empname;
	}
}
// asort($sub_tot_hr);
...
if(!empty($tbl_data)) {
	echo "<table id='project_dash' class='proj-dash-table data-table proj-table-odd-even'>
			...
			<th class='prac-dt' width='16%'>CUSTOMER TYPE</th>
				...
				foreach($user_sort_arr as $ukey=>$uval){
					...
					foreach($prj_arr as $p_name=>$pval) {
						...
						echo "<tr data-depth='".$i."' class='collapse '>
							...
							<td width='15%'>".$project_type[$p_name]."</td>
