...
//echo '<pre>'; print_r($invoiceArr); exit;
...
if(!empty($sub_tot)) {
	echo "<table id='it_cost_grid' class='data-tbl dashboard-heads dataTable it_cost_grid'>
			...
			<tr>
				...
				<th class='prac-dt'><b>GEOGRAPHY</b></th>
		...
		foreach($sort_ar as $p_name=>$user_ar) {
			...
			#differences			
			if( (!isset($sub_tot[$p_name]['invoices']) || $sub_tot[$p_name]['invoices'] == 0) && ($sub_tot[$p_name]['sub_tot_hour'] == 0) && ($sub_tot[$p_name]['sub_tot_directcost'] == 0) && empty($other_cost_val)) {
				continue;
			}
			$i       = 0;
			...
			$pjt_tot_cos			+= $sub_tot[$p_name]['invoices'];
	...
	echo "<tfoot><tr data-depth='0'>
		...
		<td align='right' class='rt-ali'><b>".round($pjt_tot_cos)."</b></td>
