	...
	?>
		...
		<div class="page-title-head">
			...
			<div class="buttons">
				<form name="fliter_data_trend" id="fliter_data_trend" method="post">
					...
					<input type="hidden" name="geography_idss" value="" id="hgeography_idss" />
		...
		<div id="filter_section">
			...
			<div id="advance_search" style="padding-bottom:15px; display:none;">
				<form action="<?php echo site_url('projects/dashboard/trend_analysis_v2')?>" name="project_dashboard" id="project_dashboard" method="post">
					<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
...
$(document).ready(function(){
  ...
  function getMembers() {
	...
	$('body').on('change','#skill_ids',function(){
		...
		var end_date   = $('#month_year_to_date').val();
		var sids = $(this).val();
		var practices_id   = $('#practice_ids').val();
		$("#filter_area_status").val('1');
		$('#member_ids').html('');
		var params = { 'dept_ids':dids,'skill_ids':sids,'start_date':start_date,'end_date':end_date,'practices_id':practices_id };
	...
	$('body').on('click','#filter_reset',function(){
		...
		$("#business_unit_id,#department_id_fk,#practices,#skill_id,#member_ids,#geography_ids").attr('selectedIndex', '-1').find("option:selected").removeAttr("selected");
	});
...
<script type="text/javascript" src="assets/js/projects/trend_analysis_view.js?v1=3"></script>
