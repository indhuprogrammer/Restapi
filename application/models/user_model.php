...
class User_model extends MY_Model {
	...
	*/
    public function user_list($offset, $search) 
    {
        if ($search != false) {
            ...
            $this->db->select('a.*, b.level_id, b.level_name, c.id, c.name, d.department as department_name, s.skillset as skill_name');
            ...
            $this->db->join($this->cfg['dbpref'].'view_econnect_department_master as d', 'd.id = a.department_id', 'left');
            $this->db->join($this->cfg['dbpref'].'view_econnect_skills_set as s', 's.id = a.skill_id', 'left');
            $this->db->where($where); 
        } else {			
	...
	/*
	*@Check Account Manager User
	*@Method  user_list
	*/
    public function checkaccountmanager($aid) 
	{
		$this->db->select('*');
		$this->db->from($this->cfg['dbpref']."customers_company as a");
		$this->db->where('customer_add',$aid);
		$query = $this->db->get();
		$customers = $query->result_array();	
		return $customers;
    }
	...
	*/
	public function insert_level_settings($level_data, $user_id, $levelId) {
		...
		$data['entity'] = $level_data['entity'];		
		if(!empty($data['entity'])) {
			for($i=0;$i<count($data['entity']);$i++){
				$dataEntity = array();
				$dataEntity['entity_id'] = $data['entity'][$i];
				$dataEntity['level_id'] = $levelId;
				$dataEntity['user_id'] = $user_id;
				$this->db->insert($this->cfg['dbpref'] . 'level_entity', $dataEntity) ;
			}
		 }

		 $data['geography'] = $level_data['geography'];		
		 if(!empty($data['geography'])) {
			 for($i=0;$i<count($data['geography']);$i++){
				 $dataGeography = array();
				 $dataGeography['geography'] = $data['geography'][$i];
				 $dataGeography['level_id'] = $levelId;
				 $dataGeography['user_id'] = $user_id;
				 $this->db->insert($this->cfg['dbpref'] . 'levels_region', $dataGeography) ;
			 }
		  } 
	...
	*/
	public function delete_level_dependant($id = null) {
		...
		$this->db->where('user_id', $id);
		$this->db->delete($this->cfg['dbpref'] . 'level_entity') ;
		$this->db->where('user_id', $id);
		$this->db->delete($this->cfg['dbpref'] . 'level_geography') ;
		$this->db->where('user_id', $id);
	...
	*/
	...
	if(!empty($data['geography'])) {	
		for($i=0;$i<count($data['geography']);$i++){				
			$dataGeography = array();
			$dataGeography['geography_id'] = $data['geography'][$i];
			$dataGeography['user_id'] = $userId;
			$dataGeography['level_id'] = $levelid; 
			if($levelid != 1) {
				$this->db->insert($this->cfg['dbpref'] . 'level_geography', $dataGeography);
			}
		}
}
		if(!empty($data['region'])) {	
	...
	*/
	...
	function get_sales_divisions() 
	{
		...
		$query = $this->db->get();
		$entity_results = $query->result();

		if(sizeof($entity_results)>0){
				foreach ($entity_results as $entity)
				{
					if($id == $entity->div_id)
						$output .= '<option value="'.$entity->div_id.'" selected = "selected" >'.$entity->division_name.'</option>';
					else
						$output .= '<option value="'.$entity->div_id.'">'.$entity->division_name.'</option>';
				}
		}
		echo $output;
   ...
   function get_geographys()
	{
		$output       	= '';
    	$this->db->select('georegionid, georegion_name');
		$this->db->where('inactive', 0);
    	$this->db->order_by('georegion_name', "asc");
		$this->db->from($this->cfg['dbpref']."geo_region");
		$query = $this->db->get();
		$geography_results = $query->result();
		if(sizeof($geography_results)>0){
			foreach ($geography_results as $geography)
			{
				if($id == $geography->georegionid)
					$output .= '<option value="'.$geography->georegionid.'" selected = "selected" >'.$geography->georegion_name.'</option>';
				else
					$output .= '<option value="'.$geography->georegionid.'">'.$geography->georegion_name.'</option>';
			}
	}
	echo $output;
    }
   ...
   public function get_loadentityByuserId($uid) {
	...
	$this->db->from($this->cfg['dbpref']."level_entity");
	$this->db->where('user_id', $uid); 
	$query  = $this->db->get();		
	$r_ids  = $query->result();
	$user_reg = array();
	foreach($r_ids as $reg_id) {
		$user_reg[] = $reg_id->entity_id;
	}
	...
	$this->db->order_by('div_id');
	$query         = $this->db->get();		
	$entity_query  = $query->result();
	
	foreach ($entity_query as $entity)
	{		
		if(in_array($entity->div_id,$user_reg)){
			$output .= '<option value="'.$entity->div_id.'" selected = "selected" >'.$entity->division_name.'</option>';
		}else{
			$output .= '<option value="'.$entity->div_id.'">'.$entity->division_name.'</option>';
		}
...
public function get_loadgeogrphyByuserId($uid) {
   
	$uid = (int)$uid; // User ID
	
	$output       	= '';
	
	$this->db->select('geography_id');
	$this->db->from($this->cfg['dbpref']."level_geography");
	$this->db->where('user_id', $uid); 
	$query  = $this->db->get();		
	$r_ids  = $query->result();
	...
	$this->db->from($this->cfg['dbpref']."geo_region");
	$query = $this->db->get();
	$geography_query  = $query->result();
	
	foreach ($geography_query as $geography)
	{		
		if(in_array($geography->georegionid,$user_reg)){
			$output .= '<option value="'.$geography->georegionid.'" selected = "selected" >'.$geography->georegion_name.'</option>';
		}else{
			$output .= '<option value="'.$geography->georegionid.'">'.$geography->georegion_name.'</option>';
		}
   ...
   */
	...
	*/
	...
	{
		$this->db->select('ul.username, ul.emp_id, ul.active, ul.created_on, r.name, d.department as department_name, s.skillset as skill_name');
		$this->db->from($this->cfg['dbpref'].'users_logs as ul');
		$this->db->join($this->cfg['dbpref'].'roles as r', 'r.id = ul.role_id', 'left');
		$this->db->join($this->cfg['dbpref'].'view_econnect_department_master as d', 'd.id = ul.department_id', 'left');
		$this->db->join($this->cfg['dbpref'].'view_econnect_skills_set as s', 's.id = ul.skill_id', 'left');
