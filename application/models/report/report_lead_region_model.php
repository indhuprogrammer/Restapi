...
class Report_lead_region_model extends MY_model {
    ...
    public function getLeadReportByRegion($options = array()) {
		...
		$this->db->join($this->cfg['dbpref'].'customers_company_address as cca','cca.customer_id = cc.companyid');
		$this->db->join($this->cfg['dbpref'].'region reg','cca.add1_region = reg.regionid','INNER');
    	$this->db->join($this->cfg['dbpref'].'country country','cca.add1_country = country.countryid','INNER');
    	$this->db->join($this->cfg['dbpref'].'state state','cca.add1_state = state.stateid','INNER');
    	$this->db->join($this->cfg['dbpref'].'location location','cca.add1_location = location.locationid','INNER');
